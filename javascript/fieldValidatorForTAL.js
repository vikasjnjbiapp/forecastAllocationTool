...
let calLastRollingForecastVolume = function(a, cust_name, country_id) {
  $('#entryGridFormCurrent tr').each(function(row){
    let itemId = $('#item_'+(row+1)).val();
    if(itemId !== undefined) {
        $.post('router.php', {page: 'lastRollingForecast', arrayData: itemId, otherdata: {customerName: cust_name, countryId: country_id} }, function(data) {
            if(data!=="")
               var dataValue = data;
             else
               var dataValue = 0;
            $('#lastRollingForecast_fcast_'+(row+1)).val(dataValue);
         });
     }
  });
};

let calForecastVolume = function(a, cust_name, country_id) {
  $('#entryGridFormCurrent tr').each(function(row){
    let itemId = $('#item_'+(row+1)).val();
    if(itemId !== undefined) {
        $.post('router.php', {page: 'totalSalesTarget', arrayData: itemId, otherdata: {customerName: cust_name, countryId: country_id} }, function(data) {
            if(data!=="")
               var dataValue = data;
             else
               var dataValue = 0;
            $('#totalSalesTarget_fcast_'+(row+1)).val(dataValue);
         });
     }
  });
};
